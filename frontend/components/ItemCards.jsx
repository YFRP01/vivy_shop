...
const ItemCards = ({item}) => {
    ...
    return (
      <div onClick={()=>(navigate(`/item/${item.item_id}`))} className='flex overflow-hidden hover:scale-101 w-full flex-col h-83 relative bg-white shadow-md hover:shadow-lg transition shadow-all duration-300 p-1 rounded-xl'>
        ...
        <div className='bg-[#e1e5e9] w-full flex-2 flex items-center justify-center text-xs text-black/40 rounded-xl'>
            <img src={item.thumbnail} alt={item.name +' image'} className='object-cover w-full flex items-center justify-center h-full transition all duration-200 px-5'/>
        </div>
        <div className='flex-1 px-2 pt-3 relative'>
        ...
        <div className='h-fit flex items-end'>
            <div className='flex gap-1 overflow-hidden items-center flex-wrap break-all font-sans text-sm text-[#0f172a] font-medium'>
                <span className='flex items-center'>
                    <span className='text-lg font-bold'>{formatPrice(item.info.cost)}</span>
        ...
        <button onClick={()=>(editItem())} className='absolute bottom-25 right-1'>
