...
const DeveloperLayout = () => {
  ...
  const methods = [
    {"id": 0, "title":"POST", "nav": ['/developer', '/developer/categories/create'] },
    {"id": 1, "title":"EDIT", "nav": ['/developer/items/list','/developer/categories/list'] }
  ]
  ...
  const [selectedPage, setSelectedPage] = useState(0)
    ...
    useEffect(()=>{
    navigate(methods[selectedMethod].nav[selectedPage])
  },[selectedPage])
        ...
        <div className='flex gap-1 h-full relative'>
          <div className='absolute top-0 w-16 md:w-26 py-4 flex flex-col gap-2 bg-gray-100 border-r h-full border-blue-200'>
            {methods.map((m)=>(
              ...
              onClick={()=>(setSelectedMethod(m.id), navigate(m.nav[selectedPage]))}>{m.title}</p>
            ))}
          </div>
