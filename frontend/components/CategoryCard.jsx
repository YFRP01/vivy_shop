...
const CategoryCard = ({cat}) => {
  ...
  return (
    <div onClick={()=>(handleClick(main, category))} className={`flex flex-col hover:scale-102 ${category === cat.value && 'border-2 scale-102 border-green-500'} h-45 w-35 md:w-42 rounded-2xl bg-black/20`}>
        <div className='flex-3 border-b border-gray-500'>
            <img src={cat.image} alt={`${cat.value} image`} className='object-cover bg-gray-400 rounded-t-2xl text-sm md:text-md text-gray-500 flex items-center justify-center w-full h-full' />
        </div>
        <div className='flex-1 flex w-full justify-center items-center'>{cat.value}</div>
    </div>
  )
}
